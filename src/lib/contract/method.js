...
export default class Method {
    ...
    async _call(types, args, options = {}, callback = false) {
        ...
        this.dtcSdk.transactionBuilder.triggerSmartContract(
            this.contract.address,
            this.functionSelector,
            options,
            parameters,
            options.from ? this.dtcSdk.address.toHex(options.from) : false,
            (err, transaction) => {
    ...
    async _send(
        ...
        try {
            const address = privateKey
                ? this.dtcSdk.address.fromPrivateKey(privateKey)
                : this.dtcSdk.defaultAddress.base58;
            const transaction =
                await this.dtcSdk.transactionBuilder.triggerSmartContract(
                    this.contract.address,
                    this.functionSelector,
                    options,
                    parameters,
                    this.dtcSdk.address.toHex(address)
                );
            ...
            const signedTransaction = await this.dtcSdk.dtc.sign(
            ...
            const broadcast = await this.dtcSdk.dtc.sendRawTransaction(
                signedTransaction
            );
            ...
            if (broadcast.code) {
                ...
                if (broadcast.message)
                    err.message = this.dtcSdk.toUtf8(broadcast.message);
                return callback(err);
            }
            ...
            const checkResult = async (index = 0) => {
                ...
                const output = await this.dtcSdk.dtc.getTransactionInfo(
                ...
                if (output.result && output.result === "FAILED") {
                    return callback({
                        error: this.dtcSdk.toUtf8(output.resMessage),
                        transaction: signedTransaction,
                        output,
                    });
                }
    ...
    async _watch(options = {}, callback = false) {
        ...
        const getEvents = async () => {
            try {
                ...
                const events =
                    await this.dtcSdk.event.getEventsByContractAddress(
                        this.contract.address,
                        params
                    );
