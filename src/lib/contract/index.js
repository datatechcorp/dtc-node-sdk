...
export default class Contract {
    constructor(dtcSdk = false, abi = [], address = false) {
        ...
        if (this.dtcSdk.isAddress(address)) this.deployed = true;
    ...
    async _getEvents(options = {}) {
        const events = await this.dtcSdk.event.getEventsByContractAddress(
            this.address,
            options
        );
    ...
    async new(
        ...
        try {
            const address = this.dtcSdk.address.fromPrivateKey(privateKey);
            const transaction =
                await this.dtcSdk.transactionBuilder.createSmartContract(
                    options,
                    address
                );
            ...
            const contract = await this.dtcSdk.dtc.sendRawTransaction(
                signedTransaction
            );
            ...
            if (contract.code)
                return callback({
                    error: contract.code,
                    message: this.dtcSdk.toUtf8(contract.message),
                });
    ...
    async at(contractAddress, callback = false) {
        ...
        try {
            const contract = await this.dtcSdk.dtc.getContract(contractAddress);
