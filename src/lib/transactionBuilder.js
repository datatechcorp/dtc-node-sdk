...
function fromUtf8(value) {
    return self.dtcSdk.fromUtf8(value);
}
...
function resultManager(transaction, callback) {
    ...
    if (transaction.result && transaction.result.message) {
        return callback(self.dtcSdk.toUtf8(transaction.result.message));
    }
...
export default class TransactionBuilder {
    ...
    sendDtc(
        to = false,
        amount = 0,
        from = this.dtcSdk.defaultAddress.hex,
        options,
        callback = false
    ) {
        ...
        if (!callback)
            return this.injectPromise(this.sendDtc, to, amount, from, options);
            ...
            this.validator.notValid(
                [
                    ...
                    },
                    {
                        ...
                        type: "notEqual",
                        msg: "Cannot transfer DTC to the same account",
                    },
                    {
                        name: "amount",
                        type: "integer",
    ...
    purchaseToken(
        ...
        tokenID = false,
        amount = 0,
        buyer = this.dtcSdk.defaultAddress.hex,
        options,
        callback = false
    ) {
        if (utils.isFunction(options)) {
    ...
    vote(
        votes = {},
        voterAddress = this.dtcSdk.defaultAddress.hex,
        options,
        callback = false
    ) {
        if (utils.isFunction(options)) {
    ...
    createSmartContract(
        options = {},
        issuerAddress = this.dtcSdk.defaultAddress.hex,
        callback = false
    ) {
        ...
        const feeLimit = options.feeLimit || this.dtcSdk.feeLimit;
        ...
        // tokenValue and tokenId can cause errors if provided when the dtc10 proposal has not been approved yet. So we set them only if they are passed to the method.
    ...
    clearABI(
        contractAddress,
        ownerAddress = this.dtcSdk.defaultAddress.hex,
        callback = false
    ) {
        if (!callback)
            return this.injectPromise(
        ...
        if (!this.dtcSdk.isAddress(ownerAddress))
        ...
        if (this.dtcSdk.dtc.cache.contracts[contractAddress]) {
            delete this.dtcSdk.dtc.cache.contracts[contractAddress];
        }
        this.dtcSdk.fullNode
    ...
    createToken(
        ...
        const {
            ...
            url = false,
            totalSupply = 0,
            dtcRatio = 1, // How much DTC will `tokenRatio` cost?
            tokenRatio = 1, // How many tokens will `dtcRatio` afford?
            saleStart = Date.now(),
            saleEnd = false,
        ...
        if (
            this.dtcSdk.fullnodeSatisfies(">=3.5.0") &&
    ...
    /**
     * Create an exchange between a token and DTC.
     * Token Name should be a CASE SENSITIVE string.
     * PLEASE VERIFY THIS ON DTCSCAN.
     */
    createDTCExchange(
        tokenName,
        tokenBalance,
        ...
        const data = {
            ...
            second_token_id: "5f", // Constant for DTC.
            second_token_balance: dtcBalance,
        };
     ...
     * Use "_" for the constant value for DTC.
     */
    ...
    tradeExchangeTokens(
        ...
        const data = {
            owner_address: toHex(ownerAddress),
            exchange_id: parseInt(exchangeID),
            token_id: this.dtcSdk.fromAscii(tokenName),
    ...
    checkPermissions(permissions, type) {
        if (permissions) {
            ...
            for (let key of permissions.keys) {
                if (
                    !this.dtcSdk.isAddress(key.address) ||
    ...
    async alterTransaction(transaction, options = {}, callback = false) {
        ...
        if (options.data) {
            if (options.dataFormat !== "hex")
                options.data = this.dtcSdk.toHex(options.data);
