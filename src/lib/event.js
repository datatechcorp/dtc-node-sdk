...
export default class Event {
    ...
    setServer(eventServer = false, healthcheck = "healthcheck") {
        if (!eventServer) return (this.dtcSdk.eventServer = false);

        if (utils.isString(eventServer))
            eventServer = new providers.HttpProvider(eventServer);

        if (!this.dtcSdk.isValidProvider(eventServer))
            throw new Error("Invalid event server provided");

        this.dtcSdk.eventServer = eventServer;
        this.dtcSdk.eventServer.isConnected = () =>
            this.dtcSdk.eventServer
                .request(healthcheck)
                .then(() => true)
                .catch(() => false);
    }
    ...
    getEventsByContractAddress(
        ...
        if (contractAddress)
            routeParams.push(this.dtcSdk.address.fromHex(contractAddress));
        ...
        if (onlyConfirmed) qs.onlyConfirmed = onlyConfirmed;
        ...
        return this.dtcSdk.eventServer
            .request(
