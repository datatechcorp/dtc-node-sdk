...
export default class SideChain {
    constructor(
        ...
        const self = this;
        this.sidechain.dtc.sign = (...args) => {
            return self.sign(...args);
        };
        this.sidechain.dtc.multiSign = (...args) => {
    ...
    async multiSign(
        ...
        if (
            ...
            const address = this.sidechain.address
                ...
                .toLowerCase();
            const signWeight = await this.sidechain.dtc.getSignWeight(
                transaction,
                permissionId
            );
    ...
    async sign(
        ...
        if (this.utils.isFunction(useDtcHeader)) {
        ...
        if (this.utils.isString(transaction)) {
            ...
            try {
                const signatureHex = this.sidechain.dtc.signString(
                    transaction,
                    privateKey,
                    useDtcHeader
                );
    ...
    async mappingTrc(
        ...
        dtcHash = dtcHash.startsWith("0x") ? dtcHash : "0x" + dtcHash;
        options = {
            feeLimit,
            ...options,
        ...
        try {
            ...
            if (functionSelector === "mappingTRC20") {
                result = await contractInstance
                    .mappingTRC20(dtcHash)
                    .send(options, privateKey);
            } else if (functionSelector === "mappingTRC721") {
                result = await contractInstance
                    .mappingTRC721(dtcHash)
                    .send(options, privateKey);
            } else {
    ...
    /**
     * withdraw dtc from sidechain to mainchain
     */
    async withdrawDtc(
        callValue,
        withdrawFee,
    ...
    async withdrawTrc(
        ...
        try {
            ...
            const signedTransaction = await this.sidechain.dtc.sign(
            ...
            const broadcast = await this.sidechain.dtc.sendRawTransaction(
                signedTransaction
            );
            if (broadcast.code) {
                const err = {
            ...
            const checkResult = async (index = 0) => {
                ...
                const output = await this.sidechain.dtc.getTransactionInfo(
