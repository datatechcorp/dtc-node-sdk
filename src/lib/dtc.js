...
const DTC_MESSAGE_HEADER = "\x19DTC Signed Message:\n32";
// it should be: '\x15DTC Signed Message:\n32';
const ETH_MESSAGE_HEADER = "\x19Ethereum Signed Message:\n32";

function toHex(value) {
    return DtcSdk.address.toHex(value);
}

export default class Dtc {
    constructor(dtcSdk = false) {
        ...
        this.validator = new Validator(dtcSdk);
    }

    _parseToken(token) {
        return {
            ...token,
            name: this.dtcSdk.toUtf8(token.name),
            abbr: token.abbr && this.dtcSdk.toUtf8(token.abbr),
            description:
                token.description && this.dtcSdk.toUtf8(token.description),
            url: token.url && this.dtcSdk.toUtf8(token.url),
        };
    }

    getCurrentBlock(callback = false) {
        if (!callback) return this.injectPromise(this.getCurrentBlock);
        this.dtcSdk.fullNode
    ...
    getBlock(block = this.dtcSdk.defaultBlock, callback = false) {
        if (utils.isFunction(block)) {
            callback = block;
            block = this.dtcSdk.defaultBlock;
        }
    ...
    _getTransactionInfoById(transactionID, options, callback = false) {
        ...
        this.dtcSdk[options.confirmed ? "solidityNode" : "fullNode"]
            .request(
                `wallet${
    ...
    getTransactionsToAddress(
        address = this.dtcSdk.defaultAddress.hex,
        limit = 30,
        offset = 0,
        callback = false
    ) {
        ...
        address = this.dtcSdk.address.toHex(address);
    ...
    async getTransactionsRelated(
        ...
        if (!this.dtcSdk.isAddress(address))
    ...
    getAccount(address = this.dtcSdk.defaultAddress.hex, callback = false) {
        if (utils.isFunction(address)) {
            callback = address;
            address = this.dtcSdk.defaultAddress.hex;
        }
    ...
    getBalance(address = this.dtcSdk.defaultAddress.hex, callback = false) {
        if (utils.isFunction(address)) {
            callback = address;
            address = this.dtcSdk.defaultAddress.hex;
        }
    ...
    getBandwidth(address = this.dtcSdk.defaultAddress.hex, callback = false) {
        if (utils.isFunction(address)) {
            callback = address;
            address = this.dtcSdk.defaultAddress.hex;
        }
    ...
    getTokenFromID(tokenID = false, callback = false) {
        ...
        this.dtcSdk.fullNode
            .request(
                ...
                {
                    value: this.dtcSdk.fromUtf8(tokenID),
                },
                "post"
            )
            .then((token) => {
    ...
    getContract(contractAddress, callback = false) {
        ...
        if (!this.dtcSdk.isAddress(contractAddress))
        ...
        contractAddress = this.dtcSdk.address.toHex(contractAddress);
    ...
    async verifyMessage(
        message = false,
        signature = false,
        address = this.dtcSdk.defaultAddress.base58,
        useDtcHeader = true,
        callback = false
    ) {
        if (utils.isFunction(address)) {
        ...
        if (utils.isFunction(useDtcHeader)) {
            callback = useDtcHeader;
            useDtcHeader = true;
        }
        ...
        if (Dtc.verifySignature(message, address, signature, useDtcHeader))
            return callback(null, true);

        callback("Signature does not match");
    }

    static verifySignature(message, address, signature, useDtcHeader = true) {
        message = message.replace(/^0x/, "");
        signature = signature.replace(/^0x/, "");
        const messageBytes = [
            ...toUtf8Bytes(
                useDtcHeader ? DTC_MESSAGE_HEADER : ETH_MESSAGE_HEADER
            ),
        ...
        const dtcAddress = ADDRESS_PREFIX + recovered.substr(2);
        const base58Address = DtcSdk.address.fromHex(dtcAddress);

        return base58Address == DtcSdk.address.fromHex(address);
    }

    async sign(
        transaction = false,
        privateKey = this.dtcSdk.defaultPrivateKey,
        useDtcHeader = true,
        multisig = false,
        callback = false
    ) {
        ...
        if (utils.isString(transaction)) {
            ...
            try {
                const signatureHex = Dtc.signString(
                    transaction,
                    privateKey,
                    useDtcHeader
                );
        ...
        try {
            if (!multisig) {
                const address = this.dtcSdk.address
                    .toHex(this.dtcSdk.address.fromPrivateKey(privateKey))
                    .toLowerCase();

                if (
                    address !==
                    this.dtcSdk.address.toHex(
    ...
    static signString(message, privateKey, useDtcHeader = true) {
        message = message.replace(/^0x/, "");
        const value = {
    ...
    async sendTransaction(
        ...
        try {
            const address = options.privateKey
                ? this.dtcSdk.address.fromPrivateKey(options.privateKey)
                : options.address;
            const transaction = await this.dtcSdk.transactionBuilder.sendDtc(
                to,
                amount,
                address
            );
    ...
    async sendToken(
        ...
        try {
            const address = options.privateKey
                ? this.dtcSdk.address.fromPrivateKey(options.privateKey)
                : options.address;
            const transaction = await this.dtcSdk.transactionBuilder.sendToken(
                to,
                amount,
                tokenID,
                address
    ...
    /**
     * Freezes an amount of DTC.
     * Will give bandwidth OR Energy and DTC Power(voting rights)
     * to the owner of the frozen tokens.
     *
     * @param amount - is the number of frozen dtc
    ...
    async freezeBalance(
        ...
        try {
            const address = options.privateKey
                ...
                : options.address;
            const freezeBalance =
                await this.dtcSdk.transactionBuilder.freezeBalance(
                    amount,
                    duration,
                    resource,
                    address,
    ...
    /**
     * Unfreeze DTC that has passed the minimum freeze duration.
     * Unfreezing will remove bandwidth and DTC Power.
     *
     * @param resource - is the type, must be either "ENERGY" or "BANDWIDTH"
     * @param options
     * @param callback
    ...
    async unfreezeBalance(
        ...
        try {
            ...
            const unfreezeBalance =
                await this.dtcSdk.transactionBuilder.unfreezeBalance(
                    resource,
                    address,
                    receiverAddress
                );
    ...
    async updateAccount(accountName = false, options = {}, callback = false) {
        ...
        try {
            const address = options.privateKey
                ...
                : options.address;
            const updateAccount =
                await this.dtcSdk.transactionBuilder.updateAccount(
                    accountName,
                    address
                );
            const signedTransaction = await this.sign(
